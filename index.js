    ...
    methods: {
      ...
      setInitDate() {
        $(".topic-list-item .age").each((index, element) => {
          const $views = $(element).siblings(".views");
          if ($views.find(".linuxtime").length > 0) {
            return;
          }
          const str = $(element).attr("title");
          const match = str.match(/创建日期：([\s\S]*?)最新：/);
...
<div class="gpt-summary">${marked.marked.parse(existingObject.value)}</div>
  ...
  const _sfc_main = {
    ...
    methods: {
      ...
      // 默认运行脚本
      runscripts() {
        if (this._linuxdoscriptsEventsBound) {
          return;
        }
        this._linuxdoscriptsEventsBound = true;
        $(document).off("click.linuxdoscriptsSetting", ".linuxdoscripts-setting").on("click.linuxdoscriptsSetting", ".linuxdoscripts-setting", function() {
          $(".linuxdoscripts-opacity").show();
          $("#menu_suspendedball").show();
        });
        $(document).off("click.linuxdoscriptsNav", ".menu-nav li").on("click.linuxdoscriptsNav", ".menu-nav li", function() {
    ...
    created() {
      ...
      setInterval(() => {
        ...
        const $actions = $(".sidebar-footer-actions");
        const $button = $(".linuxdoscripts-setting").first();
        if ($button.length < 1) {
          if ($actions.length > 0) {
            $actions.prepend(settingButton);
          } else {
            $("body").append(settingButton);
            $(".linuxdoscripts-setting").last().css({
              position: "fixed",
              right: "20px",
              bottom: "86px",
              zIndex: 1e4
        ...
        } else if ($actions.length > 0 && !$button.parent().is($actions)) {
          $actions.prepend($button.removeAttr("style"));
        }
      }, 1e3);
